  ...
  return (
    <div className="h-[100%] flex flex-col">
            ...
            {email ? (
              <li className="relative" ref={dropdownRef}>
                <div
                  ...
                  className="w-10 h-10 flex items-center justify-center bg-blue-500 hover:bg-blue-800 text-white rounded-full cursor-pointer select-none"
                >
                  {email[0].toUpperCase()}
                </div>
                {dropDownOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-300 rounded shadow-md p-4 z-30">
                    ...
                    <button
                      ...
                      className="mt-2 text-red-600 hover:underline text-sm hover:cursor-pointer"
                    >
                      Logout
                    </button>
                  </div>
      ...
      <div className="h-[calc(100%-120px)] overflow-auto">
        <Outlet context={{ setIsLoginFormOpen }} />
      </div>
    </div>
  );
};
